...
def vis_eval_result(img, window, wall, plotLine=False, saveFilename=None):
    if isinstance(img, str):
        if os.path.exists(img):
            cv_img_patch_show = cv2.imread(img)
    elif isinstance(img, np.ndarray):
    ...
    for idx in range(len(wall)):
        lt, lb, rb, rt = wall[idx]['position'][:4]
...
def vis_eval_result_with_gt(img, predWindow, gtWindow, predWall, gtWall, plotLine=False, saveFilename=None):
    if isinstance(img, str):
        if os.path.exists(img):
            cv_img_patch_show = cv2.imread(img)
    elif isinstance(img, np.ndarray):
    ...
    for idx in range(len(gtWall)):
        lt, lb, rb, rt = gtWall[idx]
  ...
  ####PRED Wall
      # PRED
    for idx in range(len(predWall)):
        lt, lb, rb, rt = predWall[idx]['position'][:4]
        score = predWall[idx]['score']
