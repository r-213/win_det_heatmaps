...
class PoseNet_3branch(nn.Module):
    def __init__(self, backbone, heatmap_head, tagmap_head, heatmap_wall):
        super(PoseNet_3branch, self).__init__()
        self.backbone = backbone
        self.heatmap_head = heatmap_head
        self.heatmap_wall = heatmap_wall
        self.tagmap_head = tagmap_head

    def forward(self, x):
        x = self.backbone(x)
        heatmap = self.heatmap_head(x)
        heatmap_w = self.heatmap_wall(x)
        tagmap = self.tagmap_head(x)

        return heatmap, tagmap, heatmap_w