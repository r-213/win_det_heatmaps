...
def get_pose_net(network_cfg, ae_feat_dim, num_point_types):
    ...
    ##wall addition
    heatmap_wall = DeconvHead(channels[-1], network_cfg.num_deconv_layers, network_cfg.num_deconv_filters,
                              network_cfg.num_deconv_kernel, network_cfg.final_conv_kernel, out_channel, network_cfg.depth_dim)                         
    ...
    elif network_cfg.head_branch == 3:
        return PoseNet_3branch(backbone_net, heatmap_head, tagmap_head, heatmap_wall)
    else:
        assert 0