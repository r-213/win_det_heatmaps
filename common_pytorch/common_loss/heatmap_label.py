...
def get_coords_from_heatmaps_with_NMS(heatmap, heatmap_w, sigma=2, param_thre1=0.1):
    assert isinstance(heatmap, heatmap_w, np.ndarray)
    ...
    for part in range(heatmap_w.shape[0]):
        map_ori = heatmap_w[part, :, :]
        map_gau = gaussian_filter(map_ori, sigma=sigma)
...
def generate_gaussian_heatmap_label_2(feat_stride, patch_width, patch_height, wall, sigma):
    bound = 3
    num_type_points = len(wall)
