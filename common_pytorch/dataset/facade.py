...
class facade(IMDB):
  ...
  ################Wall addition
def sortWall(wall1, wall2):
    if abs(wall1[1] - wall2[1]) < 3:
        return wall1[0] - wall2[0]
    else:
        return wall1[1] - wall2[1]

def calcPixelAcc(wallss, wallBoolMap, totalGTPixel):
    '''
    :param windows: N x 4 x 2
    :param winBoolMap: H x W
    :param totalGTPixel: int
    ...
    for winKpts in windows:
        try:
            cv2.fillConvexPoly(predBoolMap, wallKpts, 1)
        except Exception as e:
            assert 0, (e, wallKpts)

    predBoolMap = (predBoolMap == 1)
    interArea = np.logical_and(predBoolMap, wallBoolMap)
    correctPixelCount = interArea.sum()
...
class facade(IMDB):
    ...
    def parse_json_annotation(self, annotation_path, im_path, shape):
        ...
        for item in label['shapes']:
            if item['label'] == 'window' or 'wall':
                ...
                wall_list.append([left_top.x, left_top.y, left_bottom.x, left_bottom.y,
                                    right_bottom.x, right_bottom.y, right_top.x, right_top.y])
            else:
                print("Unknow Label Type %s in %s" % (item['label'], annotation_path))

        wall_list = sorted(wall_list, key=cmp_to_key(sortWall))
        return wall_list
    ...
    def parse_gt_file(self, data_root_path):
        ...
        for s_img_path in images_path_list:
            ...
            wall_list.append(the_window_anno)
            shape_list.append(shape)

        return img_list, wall_list, shape_list
    ...
    def gt_db(self, is_train):
        ...
        if is_train:
            img_list, wall_anno_list, shape_list = self.parse_gt_file(
                os.path.join(self.dataset_path, 'TRAIN', self.benchmark_name))
        else:
            img_list, wall_anno_list, shape_list = self.parse_gt_file(
                os.path.join(self.dataset_path, 'TEST'))
        ...
        for n_img in range(len(img_list)):
            image_path = img_list[n_img]
            the_sample_wall = np.array(wall_anno_list[n_img], dtype=np.float)
            im_height, im_width, _ = shape_list[n_img]

            if len(the_sample_wall) > max_num_walls: #exclude windows whose num exceeds max_num_windows
                continue

            left_top = the_sample_wall[:, 0: 2].copy()
            left_bottom = the_sample_wall[:, 2: 4].copy()
            right_bottom = the_sample_wall[:, 4: 6].copy()
            right_top = the_sample_wall[:, 6: 8].copy()
            center = the_sample_wall.reshape((-1, 4, 2)).mean(axis=1)

            the_sample_wall = the_sample_wall.reshape((the_sample_wall.shape[0], 4, 2))
            ...
            gt_db.append({
                ...
                'right_top': right_top,
                'center': center,
                'windows': np.array(the_sample_wall),
                'im_width': im_width,
                'im_height': im_height
            })
    ...
    @staticmethod
    def evaluate(walls_list, imdb_list, save_path, fullEval, plot):
        # pre-processing
        ap_gt = {}
        ap_pred = []
        for s_idx in range(len(walls_list)):
            im = imdb_list[s_idx]['image']

            # aggreate gt into dict
            wallGT = imdb_list[s_idx]['walls']
            numOfWalls = wallGT.shape[0]
            winGT = winGT.reshape((numOfWalls, 4, 2))

            wins_of_this_gt = list()
            for i in range(numOfWalls):
                temp = {}
                temp['position'] = wallGT[i]  # 4x2 array
                temp['is_matched'] = 0  # initialize to 0 (not matched)
                walls_of_this_gt.append(temp)
            ap_gt[s_idx] = walls_of_this_gt.copy()

            # aggreate pred into list
            wallPred = walls_list[s_idx]
            for i in range(len(winPred)):
                temp = {}
                temp['position'] = np.array(wallPred[i]['position'])[:, :2].copy()  # 4x2 array
                temp['img_id'] = s_idx  # index of image
                temp['score'] = winPred[i]['score']  # confidence
                ap_pred.append(temp)
            ...
            if plot:
                ...
                visFilename = os.path.join(save_path, 'vis', visFilename)
                # vis_eval_result(im, winPred, plotLine=True, saveFilename=os.path.join(save_path, 'vis', visFilename))
                vis_eval_result_with_gt(im, wallPred, wallGT, plotLine=True, saveFilename=visFilename)
        ...
        pklSave = False
        if pklSave:
            logging.info("Save wall into %s" % save_path)
            with open(os.path.join(save_path, 'wall.pkl'), 'wb') as fid:
                pickle.dump(walls_list, fid, pickle.HIGHEST_PROTOCOL)

        return name_value
  #############end of wall addition

    @staticmethod
    def plot(windows_list, walls_list, imdb_list, save_path):
        # pre-processing
        ap_pred_win = []
        ap_pred_wall =[]
        for s_idx in range(len(windows_list)):
        ...
        for s_idx in range(len(walls_list)):
            ...
            # aggreate pred into list
            wallPred = walls_list[s_idx]
            for i in range(len(wallPred)):
                temp = {}
                temp['position'] = np.array(wallPred[i]['position'])[:, :2].copy()  # 4x2 array
                temp['img_id'] = s_idx  # index of image
                temp['score'] = winPred[i]['score']  # confident
                ap_pred_wall.append(temp)
        

            visFilename = "vis_" + os.path.basename(im)
            visFilename = os.path.join(save_path, visFilename)
            vis_eval_result(im, winPred,wallPred, plotLine=True, saveFilename=visFilename)