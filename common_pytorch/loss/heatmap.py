...
#########Wall addition
    def forward(self, heatmaps_w, gt_heatmaps_w, gt_loc_w):
        keypoints = gt_loc_w.type(dtype=torch.int64)
        _assert_no_grad(gt_heatmaps)
        _assert_no_grad(keypoints)
        batchsize, num_points, _, _ = gt_heatmaps_w.shape

        gt_heatmaps_w = gt_heatmaps_w.reshape(batchsize, num_points, -1)
        heatmaps_w = heatmaps_w.reshape(batchsize, num_points, -1)
        hm_loss_w = self.hm_loss_w(heatmaps_w, gt_heatmaps_w)

        return hm_loss_w

# define label for wall
def generate_heatmap_label_w(config, patch_width, patch_height, wall):
    type = config.heatmap_type
    sigma = config.sigma
    feat_stride = config.feat_stride

    if 'gaussian' in type:
        return generate_gaussian_heatmap_label(feat_stride, patch_width, patch_height, wall, sigma)
    else:
        assert 0, 'Unknown heatmap type {0}'.format(type)
