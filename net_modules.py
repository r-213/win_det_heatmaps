...
def trainNet(nth_epoch, train_data_loader, network, optimizer, loss_config, loss_func, speedometer=None):
    ...
    for idx, _data in enumerate(train_data_loader):
        ...
        heatmaps_w = network(batch_data)
        del batch_data

        loss = loss_func(heatmaps, tagmaps, heatmaps_w, batch_hm_label, batch_gt_loc)
        del batch_hm_label, batch_gt_loc
        del heatmaps, tagmaps, heatmaps_w
...
##############Wall Addition 
    ...
    with torch.no_grad():
        for idx, _data in enumerate(valid_data_loader):
            ...
            if flip_test:
                batch_data_flip = flip(batch_data, dims=3)
                heatmaps_w_flip= network(batch_data_flip)
                del batch_data_flip

            del batch_data

            if len(heatmaps_w.shape) == 5:
                heatmaps_w = heatmaps_w[:, -1]
                if flip_test:
                    heatmaps_w_flip = heatmaps_w_flip[:, -1]

            loss = loss_func(heatmaps_w, batch_hm_label, batch_gt_loc)
            del batch_hm_label
            ...
            # because we are using new DataParallel, so need to gather prediction from GPUs
            if len(devices) > 1: # TODO: check
                heatmaps_w = gather(heatmaps, 0)

            if flip_test:
                if len(devices) > 1:  # TODO: check
                    heatmaps_w_flip = gather(heatmaps_w_flip, 0)
                heatmaps_w = merge_hm_flip_func(heatmaps, heatmaps_flip, flip_pair)
                del heatmaps_w_flip

            heatmaps_w_list.append(heatmaps_w)

            del heatmaps_w

    return torch.cat(heatmaps_w_list), loss_recorder.get_avg()

def evalNet_w(nth_epoch, heatmaps_w, valid_data_loader, loss_config, test_config,
            patch_width, patch_height, final_output_path):

    print("in eval")

    heatmaps_w = nn.UpsamplingBilinear2d((patch_height, patch_width)).cuda()(heatmaps_w)
    heatmaps_w = heatmaps_w.cpu().numpy().astype(float)

    imdb_list = valid_data_loader.dataset.db

    num_samples = heatmaps_w.shape[0]
    assert num_samples == len(imdb_list)

    wall_list_with_score = list()
    ############################ Group corners on TAG ##############################
    # 1. Group Corners
    parser = HeatmapParser(loss_config, test_config.useCenter, test_config.centerT, imdb_list)
    for n_s in range(num_samples):
        try:
            group_corners_wz_score = \
                group_corners_on_tags(n_s, parser, heatmaps_w[n_s], patch_width, patch_height,
                                      imdb_list[n_s]['im_width'], imdb_list[n_s]['im_height'],
                                      rectify = test_config.rectify, wallScoreThres = test_config.wallT)
            wall_list_with_score.append(group_corners_wz_score)
        except Exception as e:
            assert 0, (n_s, e, os.path.basename(imdb_list[n_s]['image']))

    # 2. Evaluate
    name_value = facade.evaluate(wall_list_with_score, imdb_list, final_output_path,
                                 test_config.fullEval, test_config.plot)
    for name, value in name_value:
        logging.info('Epoch[%d] - Validation %s=%.3f', nth_epoch, name, value)
...
def inferNet(infer_data_loader, network, merge_hm_flip_func, merge_tag_flip_func, flip_pairs,
    ...
    with torch.no_grad():
        for idx, _data in enumerate(infer_data_loader):
            ...
            if flip_test:
                heatmaps_w = merge_hm_flip_func(heatmaps, heatmaps_flip, flip_pairs)

            heatmaps_w_list.append(heatmaps)

    heatmaps_w = torch.cat(heatmaps_w_list)

    print("in infer")
    heatmaps_w = nn.UpsamplingBilinear2d((patch_height, patch_width)).cuda()(heatmaps)
    heatmaps_w = heatmaps_w.cpu().numpy().astype(float)
    ...
    for n_s in range(num_samples):
        try:
            group_corners_wz_score = \
                group_corners_on_tags(n_s, parser, heatmaps_w[n_s], tagmaps[n_s], patch_width, patch_height,
                                      imdb_list[n_s]['im_width'], imdb_list[n_s]['im_height'],
                                      rectify = test_config.rectify, wallScoreThres = test_config.wallT)
            wall_list_with_score.append(group_corners_wz_score)
        except Exception as e:
    ...
    # 2. Infer or Evaluate
    facade.plot(windows_list_with_score, imdb_list, final_output_path)
    facade.plot(wall_list_with_score, imdb_list, final_output_path)